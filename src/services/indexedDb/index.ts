...
export const postValueIndexedDb = async (
  table: IndexDbTable,
  dataToAdd: any
): Promise<boolean> => {
...
export const addPostRequestToIndexedDb = async (
  url: string,
  payload: any,
  options: APIOptions
): Promise<boolean> => {
  const pendingPostRequests = await getAllValueIndexedDb(
    IndexDbTable.postRequests
  );
  ...
  const success: boolean = await postValueIndexedDb(
    IndexDbTable.postRequests,
    newRequest
  );
